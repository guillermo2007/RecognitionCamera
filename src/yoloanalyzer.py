...
class yoloV3_analyzer:
    ...
    def __init__(self, confidence = 0.5):
        self.confidende = confidence

    def InitYoloV3(self):
            self.weight = "yolov3.weights"
    ...
    def ImageProcess(self, image, showLabels = True, showBoundingBox = True, showFPS = False):
        ...
        frame = image.copy()
        blob = cv2.dnn.blob.blobFromImage(frame, 1 / 255.0, (416, 416), swapRB=True, crop=False)
        ...
        if len(box_line) > 0:
            ...
            for i in flat_box:
                ...
                if (showLabels == True):
                    cv2.putText(frame, label, (x-10,y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color)
